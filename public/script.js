...
async function sendToGoogleSheets(data) {
    try {
        ...
        console.log('API Key:', GOOGLE_API_KEY ? 'Configurada' : 'No configurada');
        ...
        console.error('Error detallado enviando a Google Sheets:', error);
        console.error('Mensaje de error:', error.message);
        console.error('Código de error:', error.code);
        console.error('Estado de error:', error.status);
        
        // Mostrar error específico al usuario
        let errorMessage = 'Error enviando a Google Sheets';
        if (error.status === 403) {
            errorMessage = 'Error de permisos. Verifica que el spreadsheet esté público.';
        } else if (error.status === 400) {
            errorMessage = 'Error en la solicitud. Verifica la configuración.';
